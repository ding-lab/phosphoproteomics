...
diff_exp = function(m, g1, g2, name="data"){
  ...
  for (i in 1:nrow(x)){
    ...
    if (sum(!is.na(x1))<2 | sum(!is.na(y1))<2){
      stats[i,]=c(mean(x[i,], na.rm=T),mean(y[i,], na.rm=T),rep("NA",5))} 
    else{
      # t-test
      t = t.test(x1,y1)
      t.p = t$p.value
      ...
      change = t.meanx - t.meany # using iTRAQ data this is analogus to log fold change; as log(exp1/ref) - log(exp2/ref) = log(exp1/exp2)
      ...
      # return the results
      stats[i,] = c(t.meanx, t.meany, change, t.tstat, t.p, w.Wstat, w.p)
    }
  }
...
#mut = OV_mut_s; exp = OV_JHU_Pro.d # for testing
...
find_diff_exp = function(mut, exp, name="data", geneList = FALSE, fdr_cutoff=0.1){
  mut = mut[,colnames(mut) %in% colnames(exp)]
  # default to use all genes
  if (geneList == FALSE){geneList = row.names(mut)}
  ...
  for (gene in row.names(mut)){
    for (gene in geneList){
      if (sum(is.na(mut[gene,])) + sum(mut[gene,]=="silent", na.rm=T) + sum(mut[gene,]=="wt", na.rm=T) < ncol(mut)*0.98){
        ...
        wt = as.vector(c(colnames(mut[,mut[gene,] == "silent"]),colnames(mut[,mut[gene,] == "wt"])))
        mutant= as.vector(colnames(mut[, ! colnames(mut) %in% wt]))
        
        diff_x = diff_exp(m = exp, g1 = mutant, g2 = wt, name=name)
        ...
        #plot_diff_exp_violin(mut_exp, gene, name=name)
      }
    }
  }
...
plot_diff_exp_heatmap = function(fold_change, t_fdr, fdr_cutoff=0.1, name="data"){
  ...
  t_fdr$gene = rownames(t_fdr)
  fold_change.m = melt(fold_change, id.var="gene")
  t_fdr.m = melt(t_fdr, id.var="gene")
  t_fdr.m$sig = t_fdr.m$value <= fdr_cutoff
  
  fn = paste(pd, name, 'diff_exp.pdf',sep ="_")
  fdr.colors=c("NA", "#000000")
  
  p = ggplot()
  p = p + geom_tile(data=fold_change.m, aes(x=variable, y=gene, fill=as.numeric(value)), linetype="blank") + scale_fill_gradientn(name= "Average expression difference", colours=RdBu1024)#, limit=c(-2,2))
  ...
  p = p + labs(title = name, x="Mutated Gene", y="Expression") + theme_bw() + 
    theme(axis.title = element_text(size=16), axis.text.x = element_text(angle = 90, vjust = 0.5, colour="black", size=12), axis.text.y = element_text(colour="black", size=12),axis.ticks = element_blank())#element_text(colour="black", size=16))
  p = p + coord_fixed()
  p
