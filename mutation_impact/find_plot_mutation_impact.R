...
#BRCA_mut_SMG = row.names(BRCA_mut[rowSums(!(BRCA_mut == "wt")) > 5,])
# k_s_table_neighbor = k_s_table[(k_s_table$GENE %in% brcaGenes) | (k_s_table$SUB_GENE %in% brcaGenes),]
...
BRCA_Pro_c = BRCA_Pro[row.names(BRCA_Pro) %in% druggable_k,]
...
BRCA_Pho_c = BRCA_Pho[row.names(BRCA_Pho) %in% druggable_k,]
...
# k_s_table_neighbor = k_s_table[(k_s_table$GENE %in% ovGenes) | (k_s_table$SUB_GENE %in% ovGenes),]
# k_s_table_neighbor_gene = unique(c(k_s_table_neighbor$GENE,k_s_table_neighbor$SUB_GENE))
...
OV_PNNL_Pro_c = OV_PNNL_Pro[row.names(OV_PNNL_Pro) %in% druggable_k,]
OV_PNNL_Pro_diff_exp = find_diff_exp(OV_mut_g,OV_PNNL_Pro_c,name="OV_PNNL_Proteome")
...
OV_PNNL_Pho_c = OV_PNNL_Pho[row.names(OV_PNNL_Pho) %in% druggable_k,]
OV_PNNL_Pho_diff_exp = find_diff_exp(OV_mut_g,OV_PNNL_Pho_c,name="OV_PNNL_Phosphoproteome")
...
fn = paste(pd, 'merged_diff_exp_cgenes_kinase_substrate.pdf',sep ="_")
...
fn = paste(pd, 'merged_diff_exp_cgenes_kinase_substrate_fdr0.05in1.pdf',sep ="_")
...
fn = paste(pd, 'merged_diff_exp_cgenes_kinase_substrate_fdr0.05in1_p.pdf',sep ="_")
p = ggplot(data=fc_fdr_s)
#p = p + facet_grid(.~variable, drop=T, scales = "free", space = "free")
p = p + facet_wrap(~variable, nrow=1, scales = "free_x", drop=TRUE)
