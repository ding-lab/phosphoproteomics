...
## plot
fn = paste(pd, 'merged_diff_exp_druggable_genes.pdf',sep ="_")
...
fn = paste(pd, 'merged_diff_exp_druggable_genes_fdr0.1in2.pdf',sep ="_")

for (i in 3:9){
  fdr[,i] = as.numeric(as.character(fdr[,i]))
}

markers = unique(fdr[rowSums(fdr[,c(3:9)]<0.1, na.rm=T) >=1,]$Var1)
