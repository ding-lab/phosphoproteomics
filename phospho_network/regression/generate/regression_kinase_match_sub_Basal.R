...
# input according to cancer type-------------------------------------------------------------------
    ...
    pho_gdata = read.delim(paste(baseD,"pan3can_shared_data/BRCA/BRCA_PHO_by_PRO_formatted_normalized_max10NA.txt",sep=""))
    clinical <- read.delim(paste(baseD,"pan3can_shared_data/BRCA/BRCA_clinical_summary.txt",sep=""))
    basal_sample <- na.omit(colnames(clinical)[clinical[1,]=="Basal"])
    pro_data <- pro_data[,c(basal_sample,"X")]
    pho_data <- pho_data[,c(basal_sample,"X")]
    pho_gdata <- pho_gdata[,c(basal_sample,"X")]
    colx <- 19
...
table_Basal <- table_2can
## write out tables
tn = paste(baseD,"pan3can_shared_data/analysis_results/regression/table/Basal_",protein,"_substrate_regression.txt", sep="")
write.table(table_Basal, file=tn, quote=F, sep = '\t', row.names = FALSE)
