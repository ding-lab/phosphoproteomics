...
# simulate a cis KS table
simulated_substrate = sample(unique(k_s_table$Substrate),length(kinase_cis), replace =F) # 
subs = as.character(simulated_substrate)
subsKS = data.frame(cbind(subs,subs,0))
colnames(subsKS) = colnames(k_s_table)
k_s_table = subsKS
