...
# input according to cancer type-------------------------------------------------------------------
  if (cancer == "HUMAN") {
    # HUMAN
    HUMAN_rna_f = paste(baseD,"pan3can_shared_data/BRCA/BRCA_mRNA_formatted_normalized.txt",sep="")
    rna_data <- read.delim(HUMAN_rna_f)
  ...
  for (kinase in kinase_cis){
    ...
    pro_kin <- rna_data[rna_data$X == kinase,-colx]
    ...
    if(nrow(pro_kin) != 0){
      ...
      for (i in s_pho_table) {
        ...
        pho_sub_t = t(pho_sub)
        row.names(pho_sub_t) = gsub("[0-9][0-9]TCGA","01A",row.names(pho_sub_t))
        pro_kin_t = t(pro_kin)
        data1 = merge(pho_sub_t,pro_kin_t, by="row.names")
        data1 = data1[,-1]
  ...
  for (kinase in kinase_trans){
    ...
    pho_kinase_g <- rna_data[rna_data$X == kinase,-colx]
      ...
      for (substrate in k_sub){# for each substrate for one kinase
        ...
        for (i in s_pho_table) {
          ...
          if(nrow(pro_sub) != 0){
            ...
            pho_kinase_g_t = t(pho_kinase_g)
            
            data2 <- data.frame(t(rbind(pho_sub,pro_sub)))
            row.names(data2) = make.names(gsub("[0-9][0-9]TCGA","01A",row.names(data2)),unique = T)
            data2 = merge(data2,pho_kinase_g_t, by="row.names")
            data2 = data2[,-1]
  ...
  table_cis$model <- "pho_sub~rna_kin"
  tablecis <- table_cis[!is.na(table_cis$P_pro_kin),]
  
  table_trans$model <- "pho_sub~pro_sub+rna_kin"
  ...
  tn = paste(baseD,"pan3can_shared_data/analysis_results/tables/BRCA_",cancer, "_", protein,"_substrate_regression_cis_rna.txt", sep="")
  write.table(tablecis, file=tn, quote=F, sep = '\t', row.names = FALSE)
  
  tn = paste(baseD,"pan3can_shared_data/analysis_results/tables/BRCA_",cancer, "_",protein,"_substrate_regression_trans_rna.txt", sep="")
