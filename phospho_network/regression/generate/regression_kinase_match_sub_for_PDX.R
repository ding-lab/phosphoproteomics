...
# look at correlations of kinase and downstream substrates phosphorylation status for PDX dataset
...
# input according to cancer type-------------------------------------------------------------------
  ...
  rsd_list <- as.vector(pho_rsd_split$SUB_MOD_RSD)
  ...
  for (kinase in unique_kinase){
    ...
    if(nrow(pro_kinase) != 0){
      ...
      for (substrate in k_sub){# for each substrate for one kinase
        ...
        s_pho_table <- which(pho_data$gene==substrate)
        
        # go through all the phosphosites
        for (i in s_pho_table) {
          
          # find phosphorylation level
          sub_mod_rsd <- rsd_list[i]
          pho_sub <- pho_data[i,6:30-1]
          ...
          if( size > least_samples ){#more than 2 complete dataset
            ...
            pro_sub <- pro_data[pro_data$Gene == substrate,6:30]
...
## write out tables
tn = paste(baseD,"pan3can_shared_data/analysis_results/PDX/regression/table/",protein,"_substrate_regression.txt", sep="")
...
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_trans_gene_count.png',sep ="")
ggsave(file=fn, height=14, width=8)
...
ggplot() +
  ...
  xlab(protein)+ylab("number of phosphosites with different regression result")+
  coord_flip()+ theme(axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=5))
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_trans_gene_ratio.png',sep ="")
ggsave(file=fn, height=14, width=8)

# scatterplot
p <- ggplot(valid_trans, aes(all_count, valid_ratio, label = rownames(valid_trans)))
p + geom_text(check_overlap = TRUE)
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_trans_all_count~valid_ratio.png',sep ="")
...
ggplot() +
  ...
  xlab(protein)+ylab("number of phosphosites with different regression result")+
  coord_flip()+ theme(axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=5))
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_cis_gene_count.png',sep ="")
ggsave(file=fn, height=6, width=8)
...
ggplot() +
  ...
  xlab(protein)+ylab("number of phosphosites with different regression result")+
  coord_flip()+ theme(axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=5))
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_cis_gene_ratio.png',sep ="")
ggsave(file=fn, height=6, width=8)

# scatterplot
p <- ggplot(valid_cis, aes(all_count, valid_ratio, label = rownames(valid_cis)))
p + geom_text(check_overlap = TRUE)
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_cis_all_count~valid_ratio.png',sep ="")
ggsave(file=fn, height=6, width=8)
...
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_trans_residue_count.png',sep ="")
ggsave(file=fn, height=6, width=8)
...
ggplot() +
  ...
  coord_flip()+ theme(axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=10))
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_trans_residue_ratio.png',sep ="")
ggsave(file=fn, height=6, width=8)
...
ggplot() +
  ...
  xlab("residue")+ylab("number of phosphosite residues with different regression result")+
  coord_flip()+ theme(axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=10))
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_cis_residue_count.png',sep ="")
ggsave(file=fn, height=6, width=8)
...
ggplot() +
  ...
  xlab("residue")+ylab("ratio of phosphosite residues with different regression result")+
  coord_flip()+ theme(axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=10))
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/regression/overview/',protein,"/",protein,'_cis_residue_ratio.png',sep ="")
ggsave(file=fn, height=6, width=8)
...
p
fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/volcano_plots/',protein,'_substrate_volcano.pdf',sep ="")
...
for (cis in c(TRUE,FALSE)) { # loop around cis and trans
  ...
  p = p + theme(axis.title = element_text(size=10), axis.text.x = element_text(colour="black", size=6,angle=90, vjust=0.5), axis.text.y = element_text(colour="black", size=10))#element_text(colour="black", size=14))
  p
  fn = paste(baseD,'pan3can_shared_data/analysis_results/PDX/bubble_charts/',protein,'/',c,'_cis_',cis,'_fdr',sig,'_top_',top,'.pdf',sep ="")
