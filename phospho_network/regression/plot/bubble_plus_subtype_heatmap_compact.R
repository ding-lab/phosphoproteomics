...
for (cancer in "OV") {
  for (iscis in c("cis","trans")) { # loop around cis and trans
    if (iscis == "cis") {
      t0 <- table_2can[table_2can$SELF==iscis & table_2can$FDR_pro_kin <= sig & table_2can$Cancer==cancer,]
      var <- "pro_kin"
    }
    if (iscis == "trans") {
      t0 <- table_2can[table_2can$SELF==iscis & table_2can$FDR_pho_kin <= sig & table_2can$Cancer==cancer,]
      var <- "pho_kin"
    }
    if (nrow(t0) > 0){
      ...
      # plot together --------------------------------------------------------------------
      fn = paste(baseD,'pan3can_shared_data/analysis_results/bubble_charts/',protein,"/",cancer,"/subtype_phospho_and_expression_level_top",len,"genes_sitelimit",num_p,"_",iscis,"_",cancer,"_",protein,'table.pdf',sep ="")
      grid.newpage()
