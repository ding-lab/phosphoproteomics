...
protein <- "kinase"
#protein <- "phosphotase"
top <- 15 # choose top n rows for fdr for model1
num_p <- 3
KS_base = "KS"#"phosphonetwork" # new output

# input -------------------------------------------------------------------
pho_data = read.delim(paste(baseD,"pan3can_shared_data/BRCA/TCGA_Breast_BI_Phosphoproteome.phosphosite.itraq_abbrev_normlized_max10NA.tsv",sep=""))
...
# change input table for the new dataset
if (KS_base == "phosphonetwork"){
  ...
  cat("Phosphonetwork K-S pairs tested:")# all trans: 16314
  table(table_2can$Cancer)
  cat("Significant in BRCA:")
  dim(table_2can[!is.na(table_2can$FDR_pho_kin) & table_2can$FDR_pho_kin < sig & table_2can$Cancer=="BRCA",])[1]
  cat("Significant in OV:")
  dim(table_2can[!is.na(table_2can$FDR_pho_kin) & table_2can$FDR_pho_kin < sig & table_2can$Cancer=="OV",])[1]
}
...
#for (cancer in "BRCA") {
  for (iscis in c("cis","trans","cis_auto")) { # loop around cis and trans
    if (iscis == "cis_auto") {
      t0 <- table_2can[table_2can$SELF=="cis" & table_2can$FDR_pro_kin < sig & table_2can$Cancer==cancer & table_2can$coef_pro_kin > 1,]
      var <- "pro_kin"
    }
    if (iscis == "cis") {
    ...
    if (iscis == "trans") {
      t0 <- table_2can[table_2can$SELF==iscis & table_2can$FDR_pho_kin < sig 
                       & table_2can$Cancer==cancer & table_2can$coef_pho_kin > 0,]
      var <- "pho_kin"
    }
    if (nrow(t0) > 0){
      ...
      # plot together --------------------------------------------------------------------
      fn = paste(baseD,'pan3can_shared_data/analysis_results/bubble_charts/',protein,"/",cancer,"/subtype_phospho_and_expression_level_top",len,"genes_sitelimit",num_p,"_",iscis,"_",cancer,"_",protein,'_phosphonetwork_table.pdf',sep ="")
      grid.newpage()
      # pdf(fn, height = 14*(len+10)/(top+10), width = 8*(len+10)/(top+10))
      pdf(fn, height = 9*(len)/(top), width = 5.25)
