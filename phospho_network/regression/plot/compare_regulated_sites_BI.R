...
compare_cis = list(experimental = exp_cis[exp_cis %in% table_HUMAN_cis$pair], motif = networkin_wname_cis$pair[networkin_wname_cis$pair %in% table_HUMAN_cis$pair], quantitative = table_HUMAN_cis_sig$pair)

# statistical testing: nothing's significant...
cat("CIS:Enrichment of quantitative-associated pairs against others in the universe of tested pairs:", nrow(table_HUMAN_cis), "\n")
cat("Enrichment against experimental pairs\n")
run_fisher(universe = table_HUMAN_cis$pair,pathway_genes = unique(compare_cis$experimental),sig_genes = unique(table_HUMAN_cis_sig$pair))
cat("Enrichment against motif-predicted pairs\n")
run_fisher(universe = table_HUMAN_cis$pair,pathway_genes = unique(compare_cis$motif),sig_genes = unique(table_HUMAN_cis_sig$pair))
cat("Enrichment of motif against experimental pairs\n")
run_fisher(universe = table_HUMAN_cis$pair,sig_genes = unique(compare_cis$experimental),pathway_genes = unique(compare_cis$motif))

# how about cis vs autophosphorylation
PSP_auto = paste(sep=":", k_s_table_phosphosite_cis$GENE,k_s_table_phosphosite_cis$SUB_GENE,k_s_table_phosphosite_cis$SUB_MOD_RSD)
cat("CIS autophosphorylation pairs showing up in PSP\n")
run_fisher(universe = table_HUMAN_cis$pair,pathway_genes = unique(PSP_auto),sig_genes = unique(table_HUMAN_cis_sig$pair[table_HUMAN_cis_sig$coef_pro_kin>1]))
unique(table_HUMAN_cis_sig$pair[table_HUMAN_cis_sig$coef_pro_kin>1])[unique(table_HUMAN_cis_sig$pair[table_HUMAN_cis_sig$coef_pro_kin>1]) %in% PSP_auto]
...
fn = paste(baseD,'pan3can_shared_data/analysis_results/figures/Cis_kinase_substrate_exp_vs_motif_vs_quant_tested.pdf',sep ="")
pdf(file=fn)
grid.draw(venn.plot)
dev.off()

compare_trans = list(experimental = exp_trans[exp_trans %in% table_HUMAN_trans$pair], motif = networkin_wname_trans$pair[networkin_wname_trans$pair %in% table_HUMAN_trans$pair], quantitative = table_HUMAN_trans_sig$pair)

# statistical testing: okay we got significance here
cat("TRANS:Enrichment of quantitative-associated pairs against others in the universe of tested pairs:", nrow(table_HUMAN_trans), "\n")
cat("Enrichment against experimental pairs\n")
run_fisher(universe = table_HUMAN_trans$pair,pathway_genes = unique(compare_trans$experimental),sig_genes = unique(table_HUMAN_trans_sig$pair))
cat("Enrichment against motif-predicted pairs\n")
run_fisher(universe = table_HUMAN_trans$pair,pathway_genes = unique(compare_trans$motif),sig_genes = unique(table_HUMAN_trans_sig$pair))
cat("Enrichment of motif against experimental pairs\n")
run_fisher(universe = table_HUMAN_trans$pair,pathway_genes = unique(compare_trans$motif),sig_genes = unique(compare_trans$experimental))
...
fn = paste(baseD,'pan3can_shared_data/analysis_results/figures/Trans_kinase_substrate_exp_vs_motif_vs_quant_tested.pdf',sep ="")
pdf(file=fn)
grid.draw(venn.plot)
dev.off()

### wrap into upsetR
prepUpSetR = function(listOfLists){
  all = c()
  for (list in listOfLists){
    all = unique(c(all,as.character(list)))
  }
  all_data = data.frame(all)
  all_data$all = as.character(all_data$all)
  for (i in 1:length(names(compare_trans))){
    all_data[,names(compare_trans)[i]] = all_data$all %in% compare_trans[[i]]
    all_data[,names(compare_trans)[i]][all_data[,names(compare_trans)[i]]]=1
    all_data[,names(compare_trans)[i]][!all_data[,names(compare_trans)[i]]]=0
  }
  return(all_data)
}
compare_trans$observed = table_HUMAN_trans$pair
fn = paste(baseD,'pan3can_shared_data/analysis_results/figures/upset_test.pdf',sep ="")
pdf(fn, useDingbats = F)
upset(prepUpSetR(compare_trans),order.by = "freq")
dev.off()

all = c()
for (list in compare_trans){
  all = unique(c(all,as.character(list)))
}
all_data = data.frame(all)
all_data$all = as.character(all_data$all)
