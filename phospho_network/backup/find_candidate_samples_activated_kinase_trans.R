...
pro_outlier <- pro_outlier[,order(names(pro_outlier))]
...
## pre-process the k_s_table to remove those rows in which substrate==kinase
k_s_table_trans <- k_s_table[k_s_table$GENE!=as.vector(k_s_table$SUB_GENE),]

#for (kinase in "PRKCD"){#test
for (kinase in unique_kinase){
  k_k_s_table <- as.vector(k_s_table_trans$SUB_GENE[k_s_table_trans$GENE==kinase])
  for (sam in 1:length(sample_names)) {
    ...
    k_p_table[k_p_table_trans$KINASE==kinase, sam+1] <- mean(pho_data[row,sam], na.rm = TRUE)
  }
}
