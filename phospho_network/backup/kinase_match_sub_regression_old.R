  ...
  if(nrow(pro_kinase) != 0){
    ...
    for (substrate in k_sub){# for each substrate for one kinase
      ...
      for (i in s_pho_table) {
        ...
        #   list1[[length(list1)+1]] <- list(KINASE=kinase,SUBSTRATE=substrate,SUB_MOD_RSD=sub_mod_rsd,DATA=data1,Pvalue=c(coef(summary(fit1))[2,4]) )
        #   
        ...
        #       list2[[length(list2)+1]] <- list(KINASE=kinase,SUBSTRATE=substrate,SUB_MOD_RSD=sub_mod_rsd,DATA=data2,Pvalue= pvalue2)
        #       
        ...
        #           }
        #           # recording like model1 + substrate expression level
        #           list3[[length(list3)+1]] <- list(KINASE=kinase,SUBSTRATE=substrate,SUB_MOD_RSD=sub_mod_rsd,DATA=data3,Pvalue= pvalue3)
        #         }
        #       }
        #     }
        #   }
