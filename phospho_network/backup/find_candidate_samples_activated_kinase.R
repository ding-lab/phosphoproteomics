...
for (kinase in unique_kinase){
  k_k_s_table <- as.vector(k_s_table$SUB_GENE[k_s_table$GENE==kinase])
...
# calculate outlier sample number for each kinase -------------------------
...
corr_sig_phos <- cbind(phos_kin[corr_sig[,1],], phos_sub[corr_sig[,2],], FDR_corr_pho[corr_sig])
test <- corr_sig_phos[corr_sig_phos$kinase=="ERBB2" | corr_sig_phos$substrate=="ERBB2",]
