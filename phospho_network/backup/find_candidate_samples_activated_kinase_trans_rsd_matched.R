...
k_s_table_trans <- k_s_table[k_s_table$GENE!=as.vector(k_s_table$SUB_GENE),]
...
#for (kinase in unique_kinase){
  k_k_s_table <- unique(as.vector(k_s_table_trans[k_s_table_trans$GENE==kinase, c("SUB_GENE","SUB_MOD_RSD")]))
  k_sub <- as.vector(k_k_s_table$SUB_GENE)
  k_rsd <- as.vector(k_k_s_table$SUB_MOD_RSD)
  row <- c()
  for (i in 1:nrow(k_k_s_table)) {
    substrate <- k_sub[i]
    rsd <- k_rsd[i]
    row <- c(row,which(pho_rsd_split$SUBSTRATE==substrate && pho_rsd_split$SUB_MOD_RSD==rsd))
  }
...
# make violinplot for each kinase, mark Her2 -----------------------------------------
rownames(k_p_table) <- k_p_table$KINASE
range <- c("ERBB2","PRKCD") # the range of kinase we want to plot
# melt the k_p_table, add variable Her+/- according to smaple name
...
## plotting
ggplot(k_p_her, aes(x = kinase, y = ave_sub_phos, fill = Her_status)) +
...
x <- vector(mode = "logical", length = length(unique_kinase))+NaN
temp <- data.frame(matrix(rep(x,ncol(pho_data)-1), ncol=ncol(pho_data)-1, byrow=T))
...
for (i in 1:length(unique_kinase)) {
  ...
  outlierk[i,-1] = (k_p_table[i,-1] >= quantile(k_p_table[i,-1], probs=0.75, na.rm=T) + 1.5*IQR)
}
