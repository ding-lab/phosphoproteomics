      ...
      for (substrate in k_sub){# for each substrate for one kinase
        ...
        for (i in s_pho_table) {
          ...
          if( size > 2 ){#more than 2 complete dataset
            ...
            list4[[length(list4)+1]] <- list(KINASE=kinase,SUBSTRATE=substrate,SUB_MOD_RSD=sub_mod_rsd,DATA=data1,Pvalue=c(coef(summary(fit1))[2,4]),Coef_pho_kin_g=fit1$coefficients[2],Size=size)
          }
        }
      }
    }
